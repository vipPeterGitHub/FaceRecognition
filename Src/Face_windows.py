...
FaceImageQueue=multiprocessing.Queue(20)
...
def InROI(rect):
    ...
    #return True
    global Pos_Mouse,ScaleX,ScaleY
    Roi_up=Pos_Mouse[0]
...
def face_front_flag(gray,faceCascade):
    #gray=cv2.cvtColor(img,cv2.COLOR_RGB2GRAY)
...
def face_recog(img,dets,transformer):
    #global SmallFace
    gray=cv2.cvtColor(img,cv2.COLOR_RGB2GRAY)
...
#################################################################
            size_flag=True
            size_thre=96
            if(d.right()-d.left()<size_thre or d.bottom()-d.top()<size_thre):
                size_flag=False
###########################################################  
            candi_left=max(d.left()-(d.right()-d.left()),0)
            candi_top=max(d.top()-(d.bottom()-d.top()),0)
            ...
            candi_bottom=min(d.bottom()+(d.bottom()-d.top()),gray.shape[0])
            candi_img=gray[candi_top:candi_bottom,candi_left:candi_right].copy()
            color_candi_img=img[candi_top:candi_bottom,candi_left:candi_right,:].copy()
            new_left=d.left()-candi_left
            new_right=d.right()-candi_left
            new_top=d.top()-candi_top
            new_bottom=d.bottom()-candi_top
            new_rec=dlib.rectangle(new_left,new_top,new_right,new_bottom)
            shape = predictor(candi_img, new_rec)
            #feat_pca=clt_pca.transform(getallfeat(shape,color_candi_img))
            feat_pca = np.dot(getallfeat(shape,color_candi_img)-mean,compoT)
            minkey,minscore=getscore(A,G,feat_pca,db_qf)
####################################################
            angle_flag = face_front_flag(candi_img,faceCascade)
#######################################################
            sevens=np.zeros((6,2))
...
class ReadVideo(multiprocessing.Process):
    ...
    def run(self):
        ...
        if(inputtype==2):
            ...
            #capture.set(cv.CV_CAP_PROP_FRAME_WIDTH,1080)
...
class FaceRec(multiprocessing.Process):
    ...
    def run(self):
        ...
        db_qf,count,out,A,G=init_recog()
        global predictor
...
class SaveImage(multiprocessing.Process):
    def __init__(self):
        multiprocessing.Process.__init__(self)
    def run(self):
        global SaveImageValue
        SaveImageValue = multiprocessing.Value("i", 0)
        while True:
...
class Timer(QThread):
    ...
    def run(self):
        global ScaleX,ScaleY,Showcount
        ...
        mtcnn_detector = FaceDetector(minsize = 30, gpuid = 0, fastresize = False)
...
class RectTimer(QThread):
    ...
    def run(self):
        global UpdateResult,Showcount#,SmallFace,ResDict
        with QMutexLocker(self.mutex):
            self.stoped = False
        fontsize=0.8
...
readPause = multiprocessing.Value("i", 0)
UpdateResult = multiprocessing.Value("i", 0)
Showcount = multiprocessing.Value("i", 0)
ShutDownValue = multiprocessing.Value("i", 0)
SaveImageValue = multiprocessing.Value("i", 0)
UpdateValue = multiprocessing.Value("i", 0)
Pos_Mouse=multiprocessing.Array('i',6)
ScaleX = multiprocessing.Value("d", 1)
ScaleY = multiprocessing.Value("d", 1)

if __name__ == '__main__':
    #thisMac=get_mac_address()
    #targetMac="4ccc6a791e08"
    ...
    SmallFace=manager.dict()



    global ResDict
    manager_res=multiprocessing.Manager()
    ResDict=manager_res.dict()
    save_show_database(ResDict)
    '''
    readTimer = ReadVideo()
    FaceR=FaceRec()
    ...
    saveimage.start()
    while True:
        if ShutDownValue.value:
            FaceR.terminate()
            showR.terminate()
            readTimer.terminate()
            saveimage.terminate()
            del FaceR,showR,readTimer,saveimage
            break