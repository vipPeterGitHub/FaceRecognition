...
def drawrec(minkey,minscore,img,d,video_name):
    ...
    cv2.putText(img,minkey,(d.left()+sidebar,d.top()+sidebar*2),font,1,color,1,False)
    cv2.putText(img,'%2f'%(minscore),(d.left()+sidebar,d.top()+sidebar*4),font,1,color,1,False)
    cv2.putText(img,video_name,(d.left()+sidebar,d.top()+sidebar*6),font,1,(255,0,0),1,False)
...
#facePath1 = '/home/peterhou/FaceRecognition/pcaTrain/Aaron_Eckhart_0001.jpg'
#facePath2 = '/home/peterhou/FaceRecognition/pcaTrain/Aaron_Guiel_0001.jpg'
facesPath = '/home/peterhou/FaceRecognition/Simple_Src/faces/'

def get1792(facePath):
	global gshape
	...
	for k,d in enumerate(dets):
		print k,d
		gshape = predictor(gray, d)

	data = getallfeat(gshape,img)
	#print data
	#print data.shape
	return data
...
faces = sorted(faces)
print ('We have %s faces totally'%(len(faces)))
i = 0
for face in faces:
	facePath = facesPath+face
	if face == 'AJ_Cook_0001.jpg':
		data = get1792(facePath)
		i+=1
		print face
	else:
		data = np.row_stack((data,get1792(facePath)))
		i+=1
		print face
	print i
...
print data.shape
matPath = '/home/peterhou/FaceRecognition/Simple_Src/data.mat'
matNormPath = '/home/peterhou/FaceRecognition/Simple_Src/dataNorm.mat'


savemat(matPath,{'data':data})
print ('Saved mat file successfully')

#dataload = loadmat('/home/peterhou/FaceRecognition/Simple_Src/data.mat')
#print dataload['data'].shape

dataNorm = normalize(data, norm='l2')

savemat(matNormPath,{'dataNorm':dataNorm})
print ('Saved Norm mat file successfully')

pca = PCA(n_components=256)
pca.fit(dataNorm)
joblib.dump(pca, '/home/peterhou/FaceRecognition/Simple_Src/pca_model.pkl')
