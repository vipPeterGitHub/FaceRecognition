...
def mosaic(img,x1,y1,x2,y2):
    #for i in range(x1,x2):
    #    for j in range(y1,y2):
    #        img[i,j,:] = img[x1,y1,:]
    a = np.ones((40,40,3))
    a = np.multiply(img[x1,y1,:],a)
    img[x1:x2,y1:y2,:] = a
    ...
    def run(self):
        ...
        while True:
            ...
            if ret == True:
                ...
                for x1 in range(0,480,step):
                    for y1 in range(0,200,step):
                        mosaic(face,x1,y1,x1+step,y1+step) 
                for x1 in range(0,480,step):
                    for y1 in range(440,640,step):
                        mosaic(face,x1,y1,x1+step,y1+step) 
                for x1 in range(0,40,step):
                    for y1 in range(200,440,step):
                        mosaic(face,x1,y1,x1+step,y1+step) 
                for x1 in range(400,480,step):
                    for y1 in range(200,440,step):
                        mosaic(face,x1,y1,x1+step,y1+step) 
