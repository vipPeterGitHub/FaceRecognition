...
def img2feat(img,net_wholeface,net_ctf,net_le,net_re,net_eye,net_mouth,net_downmouth,mean,compoT):
    ...
    if(len(dets)==0):
        ...
        feat = np.dot(getallfeatNew(shape,img,net_wholeface,net_ctf,net_le,net_re,net_eye,net_mouth,net_downmouth)-mean,compoT)
        [x1,x2,y1,y2] = [d.left(),d.right(),d.top(),d.bottom()]#(x1,y1) --> (x2,y2)
        if x1<0:
            x1 = 10
        if y1<0:
            y1 = 10
        ...
        [x1,x2,y1,y2] = [d.left(),d.right(),d.top(),d.bottom()]#(x1,y1) --> (x2,y2)
        #print x1,x2,y1,y2
        x1 = x1 - (d.right()-d.left())*scale_for_norm
        x2 = x2 + (d.right()-d.left())*scale_for_norm
        y1 = y1 - (d.bottom()-d.top())*scale_for_norm
        y2 = y2 + (d.bottom()-d.top())*scale_for_norm
        if x1<0:
            x1 = 10
        if y1<0:
            y1 = 10
        imgFace = img[y1:y2,x1:x2,:]
        gray = cv2.cvtColor(imgFace,cv2.COLOR_RGB2GRAY)
        dets = detector(gray)
        d = dets[0]
        shape = predictor(gray, d)
        feat = np.dot(getallfeatNew(shape,imgFace,net_wholeface,net_ctf,net_le,net_re,net_eye,net_mouth,net_downmouth)-mean,compoT)
        #break
        '''
    return feat,imgFace
        

def compareFace(img1,img2,net_wholeface,net_ctf,net_le,net_re,net_eye,net_mouth,net_downmouth,mean,compoT,A,G):
    begin  = time.clock()
    feat1,face1= img2feat(img1,net_wholeface,net_ctf,net_le,net_re,net_eye,net_mouth,net_downmouth,mean,compoT)
    print face1.shape
    feat2,face2= img2feat(img2,net_wholeface,net_ctf,net_le,net_re,net_eye,net_mouth,net_downmouth,mean,compoT)
    score = get_ratios(A,G,feat1,feat2)
    end = time.clock()
    print ("compare face time is {}".format(end - begin))
    #else:
    #    score = 0
    print score
    return score,face1
