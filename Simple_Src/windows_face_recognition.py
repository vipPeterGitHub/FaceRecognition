...
###################################################################
fileRes = r'D:/face_recognition/Testdata/new_db_qf.txt'
...
# Import Lab database
f = open('../Models/db_20170425_lab_qf_new.pickle','rb')
db_qf=pickle.load(f)
f.close()
...
while capture.read():
    ...
    if isinstance(img, types.NoneType):
        ...
        img=cv2.resize(img,(1024,768))
        cv2.imshow('Face Verification',img)
        #impa = 'D:/face_recognition/Simple_Src/shortcuts/'+str(num)+'.png'
        #cv2.imwrite(impa,img)
        cv2.waitKey(10)
        #out.write(img)
capture.release()
